...
# 计算热导率（x方向）  
kappa_x_1 = ther1_kappa[:, 0] + ther1_kappa[:, 1]  
kappa_x_2 = ther2_kappa[:, 0] + ther2_kappa[:, 1]  
...
# 计算总的平均线  
avg_kappa_total = np.mean([kxi_ave_1, kxi_ave_2, kxi_ave_3], axis=0)  

# 计算标准误  
rtc_x = np.array([kxi_ave_1, kxi_ave_2, kxi_ave_3])  
rtc_x_ave = np.mean(rtc_x, axis=0)  
rtc_x_err = np.std(rtc_x, axis=0, ddof=1) / np.sqrt(len(rtc_x))  # M = len(rtc_x)  

# 计算 kappa 值及其误差  
N = len(rtc_x_ave)  
kappa_x = np.mean(rtc_x_ave[N//5:2*N//5])  # 对应MATLAB的: 1*end/5:2*end/5  
kappa_err_x = np.mean(rtc_x_err[N//5:2*N//5])  
...
plt.plot(t, kxi_ave_2, 'gray', linewidth=2, alpha=0.5)  
